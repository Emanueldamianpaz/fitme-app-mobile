...
public class SaxHandler extends DefaultHandler {
    ...
    public void characters(char[] ch, int start, int length) throws SAXException {
        ...
        if (inTag) {
            textReadied = new String(ch, start, length);
        }
    }
    ...
    public void endElement(String uri, String localName, String name) throws SAXException {
        ...
        if (inTag) {
            if (localName.equals("coordinates")) {
                ...
                try {
                    double latitude = Double.parseDouble(textReadied.substring(0, textReadied.indexOf(',')));
                    double longitude = Double.parseDouble(textReadied.substring(textReadied.indexOf(',') + 1, textReadied.lastIndexOf(',')));
                    double altitude = Double.parseDouble(textReadied.substring(textReadied.lastIndexOf(',') + 1, textReadied.length()));
                    ...
                    Log.e("SaxHandler", "Skipping pointTracked incorrect: " + textReadied);
                }
            }
